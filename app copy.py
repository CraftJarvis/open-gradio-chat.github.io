...
def parse_point_from_message(partial_message):
    # 定义正则表达式
    pattern = r'<point x="(?P<x>[\d.]+)" y="(?P<y>[\d.]+)" alt="(?P<alt>[^"]+)">(?P<content>[^<]+)</point>'
...
def draw_point_on_image(image_path, point, folder):
    # 加载原始图片
    ...
    x_pixel = int(x / 100 * width)
    y_pixel = int(y / 100 * height)

    # 绘制标记点
    draw.ellipse((x_pixel - 5, y_pixel - 5, x_pixel + 5, y_pixel + 5), fill="red", outline="black")
    draw.text((x_pixel + 10, y_pixel), alt, fill="black")

    # 保存标记后的图片
    imgid = random.randint(0, 1000000)
    marked_image_path = Path(str(folder+f"{imgid}.png"))
...
def predict(message, history, model, model_url, api_key, temp, max_output_tokens):
    ...
    if point := parse_point_from_message(partial_message):
        marked_image_paths.append(draw_point_on_image(image_path, point, folder))

    # 添加数据行
    table.add_row(datetime.now().strftime("%Y-%m-%d %H:%M:%S"), model, str(history), str(message), partial_message)
    console.print(table)

    if len(marked_image_paths)>0:
        return partial_message, gr.Image(marked_image_paths[0])
    else:
...
with gr.Blocks(fill_height=True) as demo:
    with gr.Row():
        with gr.Column(scale=3):
            ...
            #     interactive=True,
            #     label="Model")
            model = gr.Textbox(value="molmo-7b-d-0924", label="Model name")
            model_url = gr.Textbox(value="http://127.0.0.1:9161/v1", label="Model URL")
        ...
        image = gr.Image(scale = 3)
        with gr.Column(scale=8):
            chatbot = gr.Chatbot(height=650)
            multimodaltextbox.render()
            with gr.Row():
                image_gallery = gr.Gallery(label="Marked Images", show_label=True, height=200)
            gr.ChatInterface(predict, chatbot=chatbot, textbox=multimodaltextbox, multimodal=True,
                            additional_inputs=[model, model_url, api_key, temp, max_output_tokens], additional_outputs = [image], fill_height=True)
